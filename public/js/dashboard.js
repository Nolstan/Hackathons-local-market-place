...
async function loadShopDetails() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Handle undefined/null as true (default) to prevent accidental disabling
            activeStatusToggle.checked = data.data.canPlaceOrders;
            console.log('Shop details loaded. canPlaceOrders Status:', activeStatusToggle.checked);
        }
    } catch (error) {
        console.error('Error loading shop details:', error);
    } finally {
        activeStatusToggle.disabled = false;
    }
}
...
async function toggleOrderStatus() {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch(`${API_URL}/business/toggle-order-status`, {
            method: 'PUT',
            headers: {
        ...
        if (data.success) {
            ...
            activeStatusToggle.checked = !activeStatusToggle.checked;
